...
public class SyaryoTemplate{
    ...
    public void add(String key, String[] s){
        ...
        if(key.equals("詳細")) addDetail(s[0], s[1], s[2]);
        if(key.equals("生産")) addBorn(s[0], s[1]);
        if(key.equals("出荷")) addDeploy(s[0]);
        if(key.equals("新車")) addNew(s[0], s[1], s[2], s[3], s[4], s[5]);
        if(key.equals("中古")) addUsed(s[0], s[1], s[2], s[3], s[4], s[5]);
        if(key.equals("廃車")) addDead(s[0], s[1], s[2]);
        if(key.equals("最終更新日")) addLast(s[0], s[1], s[2]);
        if(key.equals("顧客")) addOwner(s[0], s[1], s[2], s[3], s[4], s[5]);
        if(key.equals("SMR")) addSMR(s[0], s[1], s[2], s[3]);
        if(key.equals("経歴")) addHistory(s[0], s[1], s[2], s[3]);
        if(key.equals("受注")) addOrder(s[0], s[1], s[2], s[3], s[4], s[5], s[6], 
                                        s[7], s[8], s[9], s[10], s[11], s[12], s[13], 
                                        s[14], s[15], s[16], s[17], s[18], s[19], s[20], 
                                        s[21], s[22], s[23]);
        if(key.equals("作業")) addWork(s[0], s[1], s[2], s[3], s[4], s[5], s[6], 
                                        s[7], s[8], s[9], s[10], s[11], s[12], s[13], 
                                        s[14], s[15], s[16], s[17]);
        if(key.equals("部品")) addParts(s[0], s[1], s[2], s[3], s[4], s[5], s[6], s[7], s[8], s[9]);
        if(key.equals("国")) addCountry(s[0], s[1], s[2], s[3]);
        if(key.equals("GPS")) addGPS(s[0], s[1], s[2], s[3], s[4]);
        if(key.equals("KMSMR")) addKMSMR(s[0], s[1], s[2], s[3]);
        if(key.equals("KMENGINE")) addKMEngine(s[0], s[1], s[2], s[3]);
        if(key.equals("KMERROR")) addKMError(s[0], s[1], s[2], s[3], s[4]);
