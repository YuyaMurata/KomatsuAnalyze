    ...
    public static void extractAccident(Map<String, SyaryoObject2> syaryoMap, PrintWriter csv) {
        ...
        for (SyaryoObject2 syaryo : syaryoMap.values()) {
            ...
            List<List> accident = syaryo.getOrder().values().stream()
                .filter(s -> s.get(SyaryoElements.Order.Summary.getNo()).toString().contains("横")
                || s.get(SyaryoElements.Order.Summary.getNo()).toString().contains("転")
                || s.get(SyaryoElements.Order.Summary.getNo()).toString().contains("水")
                || s.get(SyaryoElements.Order.Summary.getNo()).toString().contains("没"))
            ...
            if (syaryo.getError() != null) {
                for (String date : syaryo.getError().keySet()) {
                    error.put(Integer.valueOf(date.replace("/", "")),
