...
public class EvaluateSyaryo {
    ...
    public static void evalSyaryoMap(Map<String, SyaryoObject> map) {
        ...
        Map<String, List<String>> cidResult = ras.entrySet().stream()
                .collect(Collectors.toMap(
                        r -> r.getKey(),
                        r -> Arrays.asList(new String[]{rm.get(r.getKey()).toString(), ru.get(r.getKey())==null?"0":ru.get(r.getKey()).toString(), r.getValue().toString()})
                ));
    ...
    private static Map<String, Integer> use(Map map) {
        ...
        //例外処理
        Map m1 = nullCheck(key, map);
        
        eval.evaluate(key, m1);
        Long evalstop = System.currentTimeMillis();
    ...
    private static Map nullCheck(String key, Map<String, SyaryoObject> map) {
        return map.entrySet().stream()
                .filter(e -> e.getValue().get(key) != null)
                .collect(Collectors.toMap(e -> e.getKey(), e -> e.getValue()));
    }
